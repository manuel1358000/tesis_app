...
class PublicacionProvider {
  ...
  final _generalStreamController =
      StreamController<List<PublicacionModel>>.broadcast();
  final _usuarioStreamController =
      StreamController<List<PublicacionModel>>.broadcast();

  Function(List<PublicacionModel>) get generalSink =>
      _generalStreamController.sink.add;
  Function(List<PublicacionModel>) get usuarioSink =>
      _usuarioStreamController.sink.add;

  Stream<List<PublicacionModel>> get generalStream =>
      _generalStreamController.stream;
  Stream<List<PublicacionModel>> get usuarioStream =>
  ...
  Future<List<PublicacionModel>> getGeneral(int cui) async {
    try {
      ...
      final resp = await http.get(
          'http://' +
              _url +
              ':3002/get/publicacionGAU?PAGINACION=' +
              _paginacionGeneral.toString(),
  ...
  Future<List<PublicacionModel>> getMarcador() async {
    try {
      final resp = await http.get(
          'http://' + _url + ':3002/get/publicacionGAU?PAGINACION=-13',
          headers: {"Content-Type": "application/json"});
      final decodedData = json.decode(resp.body);
      if (decodedData['codigo'] == 200) return null;
      final publicaciones = new Publicaciones.fromJsonList(decodedData['data']);
  ...
  Future<List<PublicacionModel>> getUsuario(int cui) async {
    try {
      ...
      final resp = await http.get(
          'http://' +
              _url +
              ':3002/get/publicacionUAU?PAGINACION=' +
              _paginacionUsuario.toString() +
              '&CUI=' +
              cui.toString(),
          headers: {"Content-Type": "application/json"});
      final decodedData = json.decode(resp.body);
  ...
  Future<Map<String, dynamic>> eliminarPublicacion(int codPublicacion) async {
    final resp = await http.delete(
        'http://' +
            _url +
            ':3002/delete/publicacionAU?COD_PUBLICACION=' +
  ...
  Future<Map<String, dynamic>> editarPublicacion(
    ...
    final resp = await http.put('http://' + _url + ':3002/put/publicacionAU',
