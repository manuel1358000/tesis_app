...
class _EditarUsuarioPageState extends State<EditarUsuarioPage> {
  ...
  Widget _crearFondo(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Color.fromRGBO(42, 26, 94, 1.0)),
    );
  }
    ...
    return Container(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          labelStyle:
              TextStyle(fontSize: 12.0, color: Color.fromRGBO(42, 26, 94, 1.0)),
        ),
        onChanged: (value) => usuariomodel.cui = int.parse(value),
        validator: (value) {
          if (value.isEmpty) {
            return 'Ingrese un dpi/carné valido';
          }
          return null;
        },
      ),
  ...
  Widget _crearPASSWORD(BuildContext context) {
    return Container(
      ...
      child: TextFormField(
        ...
        obscureText: true,
        style: TextStyle(color: Color.fromRGBO(42, 26, 94, 1.0)),
        decoration: InputDecoration(
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(20.0)),
          fillColor: Colors.black26,
  ...
  _editar(UsuarioModel usuario, BuildContext context) async {
    if (usuario.password == usuario.confirmacion) {
      if (foto == null) {
        Map info = await usuarioProvider.editarUsuario(
            usuario.cui, usuario.password, usuario.nombre, usuario.imagen);
        if (info['codigo'] == 200) {
          _prefs.nombre = usuario.nombre;
          _prefs.password = usuario.password;
          final Data data = new Data(
              contenido: 'Los datos se actualizaron de manera correcta');
        ...
        if (urlimagen != null) {
          Map info = await usuarioProvider.editarUsuario(
              usuario.cui, usuario.password, usuario.nombre, urlimagen);
          if (info['codigo'] == 200) {
            _prefs.nombre = usuario.nombre;
            _prefs.password = usuario.password;
            final Data data = new Data(
  ...
  Widget _mostrarFoto() {
    ...
    return FadeInImage.assetNetwork(
      ...
      image: 'http://' + _url + ':3002/' + usuariomodel.imagen,
    );
  }
