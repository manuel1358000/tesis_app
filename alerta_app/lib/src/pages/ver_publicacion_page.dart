...
class VerPublicacionPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: Stack(
          children: <Widget>[_crearFondo(context), _crearInformacion(context)],
        ));
  }
  ...
  Widget _crearFlutterMap(BuildContext context) {
    return Container(
      ...
      width: 400,
      child: FlutterMap(
          options: MapOptions(center: LatLng(14.611468, -90.545515), zoom: 18),
          layers: [_crearMapa(), _crearMarcador()]),
    );
  }
  ...
  _crearMapa() {
    return TileLayerOptions(
        ...
        additionalOptions: {
          'accessToken':
              'pk.eyJ1IjoibWFudWVsMTM1ODAiLCJhIjoiY2syYmZ1MGtxMDM5bjNscGpnemRvaHZ0eiJ9.BYcnZMXkIKvvH52ijm9XvA',
          'id': 'mapbox.streets'
  ...
  Widget _crearFechaHora() {
    String dateWithT = publicacionData.fechahora.substring(0, 10) +
        publicacionData.fechahora.substring(10);
  ...
  Widget _crearTipo() {
    return Column(
      children: <Widget>[
        ...
        Text(publicacionData.tipo == 1 ? "Alerta" : "Evento".toString(),
            style: TextStyle(
                color: Colors.white,
                fontSize: 20.0,
                fontWeight: FontWeight.bold)),
  ...
  Widget _iconoDropdown(int tipo) {
    ...
    switch (tipo.toString()) {
      case "1":
        return Icon(Icons.local_hospital, color: Colors.red, size: tam);
      case "2":
        return Icon(Icons.directions_run, color: Colors.blue, size: tam);
      case "3":
        return Icon(Icons.directions_run, color: Colors.blue, size: tam);
      case "4":
        return Icon(Icons.local_hospital, color: Colors.red, size: tam);
      case "5":
        return Icon(Icons.block, color: Colors.green, size: tam);
      case "6":
        return Icon(Icons.priority_high, color: Colors.green, size: tam);
      case "7":
        return Icon(Icons.local_library, color: Colors.green, size: tam);
      case "8":
        return Icon(Icons.local_library, color: Colors.green, size: tam);
      case "9":
        return Icon(Icons.insert_comment, color: Colors.red, size: tam);
      case "10":
        return Icon(Icons.security, color: Colors.red, size: tam);
      case "11":
        return Icon(Icons.sms, color: Colors.blue, size: tam);
      case "12":
        return Icon(Icons.sms, color: Colors.blue, size: tam);
      case "13":
        return Icon(Icons.directions_bike, color: Colors.purple, size: tam);
      case "14":
        return Icon(Icons.supervisor_account, color: Colors.purple, size: tam);
      case "15":
        return Icon(Icons.security, color: Colors.red, size: tam);
      default:
        return Icon(Icons.outlined_flag, color: Colors.yellow, size: tam);
    }
  }
}
